...
namespace continuous_lunar_lander {
    ...
    public:
        PPO(
            ...
            double target_kl = 0,
            std::string output_path = "data/continuous_lunar_lander/rlop_ppo",
        ...
        std::unique_ptr<rlop::PPOPolicy> MakePPOPolicy() const override {
            auto ret = std::make_unique<PPOPolicy>(rollout_buffer_->observation_sizes()[0], rollout_buffer_->action_sizes()[0]);
            ret->Reset();
            return ret;
        }
        ...
        std::array<torch::Tensor, 3> Step(const torch::Tensor& action) override {
            torch::Tensor clipped_action = torch::clamp(action, -1, 1);
            auto [observation, reward, terminated, truncated, info] = env_.Step(rlop::gym_utils::TensorToArray(clipped_action)); 
