...
int main() {
	...
	std::vector<MultiArmedBanditSolver*> solvers;
	solvers.push_back(new EpsilonGreedySolver("epsilon_greedy", &env));
	solvers.push_back(new SoftmaxSolver("softmax", &env));
	solvers.push_back(new UCB1Solver("ucb1", &env));
	solvers.push_back(new PersuitSolver("persuit", &env));
	solvers.push_back(new PursuitEpsilonGreedySolver("persuit_epsilon_greedy", &env));
